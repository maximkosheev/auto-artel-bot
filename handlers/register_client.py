...
class RegisterClient(StateGroup):
    name = State()
    phone = State()
...
async def register_client_step2(message: Message, state: FSMContext):
    ...
    await message.answer(text="Спасибо. Теперь напишите телефон для связи")
...
async def register_client_complete(message: Message, state: FSMContext):
    ...
    try:
        ...
        register_data = await state.get_date()
        await service.register(client={
            "name": register_data.name,
            "phone": register_data.phone,
            "telegram_id": message.from_user.id
        })
        message.answer(text="Спасибо за регистрацию. Для продолжения работы воспользуйтесь меню это бота")
